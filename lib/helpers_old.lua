...
function helpers.drawslice (ox, oy, rad, angle, inverse, alpha)
  ...
  love.graphics.line(
    0, 0,
    (cs.p.paddleDistance + cs.extend) * math.cos(15 * math.pi / 180),
    (cs.p.paddleDistance + cs.extend) * math.sin(15 * math.pi / 180)
  )
  love.graphics.line(
    0, 0,
    (cs.p.paddleDistance + cs.extend) * math.cos(-15 * math.pi / 180),
  ...
  -- draw the paddle
  local paddleAngle = 30 / 2 * math.pi / 180
  love.graphics.arc('line', 'open', 0, 0, (cs.p.paddleDistance + cs.extend), paddleAngle, -paddleAngle)
  love.graphics.arc('line', 'open', 0, 0, (cs.p.paddleDistance + cs.extend) + cs.p.paddleWidth, paddleAngle, -paddleAngle)
  love.graphics.line(
    (cs.p.paddleDistance + cs.extend) * math.cos(paddleAngle),
    (cs.p.paddleDistance + cs.extend) * math.sin(paddleAngle),
    ((cs.p.paddleDistance + cs.extend) + cs.p.paddleWidth) * math.cos(paddleAngle),
    ((cs.p.paddleDistance + cs.extend) + cs.p.paddleWidth) * math.sin(paddleAngle)
  )
  love.graphics.line(
    (cs.p.paddleDistance + cs.extend) * math.cos(-paddleAngle),
    (cs.p.paddleDistance + cs.extend) * math.sin(-paddleAngle),
  ...
  helpers.color(1)
  love.graphics.circle("fill",p[1],p[2],4+cs.p.extend/2)
  helpers.color(2)
  love.graphics.circle("line",p[1],p[2],4+cs.p.extend/2)
